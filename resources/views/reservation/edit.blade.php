        ...
        <form action="/rsvp/{{ $data->id }}" method="POST" class="max-w-[640px] w-full">
          ...
          @method('PATCH')
            <main class="bg-slate-100 border border-slate-200 p-4">
                ...
                <small
                    class="mt-2 mb-5 text-[12px] italic mx-auto text-center block">{{ $data->restaurant_address }}</small>
                ...
                <button
                    class="block w-full py-1.5 border-rose-500 text-rose-500 border font-bold text-center hover:bg-rose-600 transition-all mb-5">Cancel</button>
                <button type="submit"
                    class="block w-full py-1.5 bg-rose-500 text-white font-bold text-center hover:bg-rose-600 transition-all">Edit!</button>
            </main>
        </form>
    </div>
</body>
